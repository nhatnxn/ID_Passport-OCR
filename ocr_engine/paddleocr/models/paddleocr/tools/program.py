...
def train(config,
    ...
    for epoch in range(start_epoch, epoch_num + 1):
        ...
        if dist.get_rank() == 0 and epoch > 0 and epoch % save_epoch_step == 0:
            labels = last_batch[1].astype("int32")
            label_lengths = last_batch[2].astype('int64')
            p , l = post_process_class(last_pred, labels)
            fp = p[:10]
            fl = l[:10]
...
def eval(model, valid_dataloader, post_process_class, eval_class,
    ...
    with paddle.no_grad():
        ...
        for idx, batch in enumerate(valid_dataloader):
            ...
            p = [i[0] for i in post_result[0]]
            l = [i[0] for i in post_result[1]]
    ...
    pbar.close()
    model.train()
    metric['fps'] = total_frame / (total_time+0.000001)
    return metric
